...
:wimDoWI
wimlib-imagex extract "%target%" %1 \Windows\System32\config\SOFTWARE --dest-dir=.\temp
wimlib-imagex extract "%target%" %1 \Windows\System32\slc.dll --dest-dir=.\temp
reg load HKLM\wSOFTWARE "temp\SOFTWARE"
reg delete "%IFEO%" /v DevOverrideEnable /f
reg delete "%IFEO%\msiexec.exe" /f
reg add "%_SxS%" /v PreferExternalManifest /t REG_DWORD /d 1 /f
reg unload HKLM\wSOFTWARE
type nul>wimupdt.txt
>>wimupdt.txt echo add 'temp\slc.dll' '\Windows\System32\slcmsi.dll'
>>wimupdt.txt echo add 'temp\SOFTWARE' '\Windows\System32\config\SOFTWARE'
>>wimupdt.txt echo add '%xOS%\msislc.dll' '\Windows\System32\msislc.dll'
for /f %%# in ('dir /b %xOS%\*.Manifest') do (
>>wimupdt.txt echo add '%xOS%\%%#' '\Windows\System32\%%#'
)
if %xOS%==x64 (
>>wimupdt.txt echo add 'x86\msislc.dll' '\Windows\SysWOW64\msislc.dll'
for /f %%# in ('dir /b x86\*.Manifest') do (
>>wimupdt.txt echo add 'x86\%%#' '\Windows\SysWOW64\%%#'
)
)
wimlib-imagex update "%target%" %1 < wimupdt.txt
del /f /q wimupdt.txt
